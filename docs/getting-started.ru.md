...
Откройте ссылку из раздела **Мои ссылки** (*My Links*), и её страница
покажет, что сделал именно этот код: куда он ведёт, сколько раз по нему
перешли и QR-квадрат для короткого адреса:

<img src="media/link-qr.png" alt="Страница одной ссылки: назначение, QR-код короткого адреса, счётчики переходов и график посещений" width="820">

Квадрат кодирует **короткий** URL, а не назначение, — именно это делает
напечатанный код честным: переход по-прежнему считается, срок жизни
по-прежнему действует, а удаление ссылки останавливает все отпечатанные
копии. Это SVG, и он отдаётся сам по себе по адресу
`/api/v1/links/<code>/qr`, так что плакату или слайду панель открывать не
нужно.
...
не стек, а локальный сервер. Отличить их можно по `"cache"`: у стека
`"ok"` (за ним Redis), у локального сервера `"in_process"`.
...
Локально тот же запрос отвечает `"cache": "in_process"` и `"task_queue":
"inline"`: профиль разработки держит кэш в этом процессе, а не в Redis, и
делает фоновую работу прямо на запросе. Ни то, ни другое не поломка, и
`in_process` — не `disabled`: последнее означает, что не кэшируется ничего
(`CACHE_ENABLED=false`). Различение стоит отдельного слова: записи, лежащие
в процессе, не видит ни один другой работник, поэтому ссылка, удалённая
через одного, продолжает перенаправлять через остальных, пока не выйдет её
срок в кэше.
...
даётся без учётной записи и сколько они живут — по умолчанию десять и семь
дней. Десять — это сколько гость может **держать** внутри окна, а не
сколько может создать: удаление освобождает место сразу же, а истёкшая
ссылка держит своё место, пока её не удалят. Только что созданную ссылку
можно тут же удалить: гостевой ссылке нечем доказать владение, кроме
токена, выданного вместе с ней.
