...
то есть почта включена, а сервера, куда её слать, нет. Регистрация всё
равно отвечает `202`, письмо никуда не уходит, а в журнале появляется
`Verification email not delivered`. Чтобы ловить письма локально, поднимите
Mailpit из докеровского стека и задайте `MAIL_HOST=localhost` и
`MAIL_PORT=1025`: без своего порта профиль берёт умолчание `587`, а
приёмник слушает не его.
...
> мимо: поле тела или параметр строки запроса, которого операция не
> объявляет, получают `400 VALIDATION_ERROR` с именем виновника —
> `'bogus' is not a parameter of this endpoint`. Поэтому опечатка
> `?short_code=` не вернётся цифрами по всей службе. На страницы правило
> не распространяется: навигация несёт что угодно из адресной строки.
