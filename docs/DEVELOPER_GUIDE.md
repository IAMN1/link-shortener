...
Тесты: 1869 (unit + integration + e2e), покрытие: 91.09% при пороге 88%
...
точки: `tests/unit` → `tests/build` даёт молча 619 вместо 1869). Два
...
оставляет сбор на 1869 и коде 0, и краснеет уже сам прогон.
...
- **Проверка адреса — проверка формы, а не адреса.** Правило требует
  ...
  `a@b.c` проходит, несуществующий домен проходит, существование ящика не
  проверяется ничем — почтового канала пока нет
