...
Тесты: 1684 (unit + integration + e2e), покрытие: 90.81% при пороге 88%
...
точки: `tests/unit` → `tests/build` даёт молча 556 вместо 1684). Два
...
оставляет сбор на 1684 и коде 0, и краснеет уже сам прогон.
...
**Тесты с временными допусками на общем железе.** Их одиннадцать, и три из
...
| `tests/unit/infrastructure/test_failover/test_failover_service.py:520` | `test_a_call_in_flight_does_not_hold_up_a_reader` — читатель обязан уложиться в 0.05 с, пока рядом идёт вызов на 0.3 с |
| `tests/unit/infrastructure/test_failover/test_failover_service.py:553` | `test_two_calls_do_not_queue_behind_one_another` — два вызова по 0.2 с обязаны уложиться в 0.3 с на двоих |
| `tests/unit/infrastructure/test_failover/test_failover_service.py:413` | `test_threads_failing_at_once_do_not_skip_a_healthy_standby` — окно 0.02 с на то, чтобы четыре потока успели войти |
| `tests/unit/infrastructure/test_failover/test_failover_service.py:878` | `test_a_climb_is_decided_on_the_list_as_it_is_at_the_time` — проба спит 0.2 с, за это время в неё должен успеть вклиниться вызов |
...
41 набор поймал, 49 пережили полный прогон. Четыре из переживших закрыты в
тот же день (`status_reader` и ширина колонки роли), остальные записаны
здесь — не как дефекты кода, а как места, где сломать можно молча.

Дыры сгруппированы, а не рассыпаны, и групп четыре.

**Что менеджеры делают с `FailoverService`, не проверяет ничто.** Все
тестовые конфигурации выключают журналирование и аудит, поэтому ветка с
failover в менеджерах не исполняется ни разу. Пережили набор: возврат
`_active_audit_logger` вместо прокси (при `AUDIT_TYPE=auto` это `None`,
то есть `AttributeError` и 500 на создании ссылки и на переходе по ней);
health-checker, всегда отвечающий «здоров» (понижения не происходит
никогда — а это единственный отказ, ради которого класс написан);
`check_interval=None` (фоновый поток не стартует, и работа не возвращается
наверх никогда); кулдаун 30 секунд вместо пяти минут. Проверено руками:
`AuditManager(audit_type="auto")` действительно строит failover и держит
`_active_audit_logger is None`.

**`/api/v1/admin/health` проверяется на данных, где все поля равны.**
`test_get_health` ставит `database`, `cache`, `task_queue` и
`rate_limiter` в `True`, поэтому подмена любого поля любым другим
неразличима: упавший Redis показывается здоровым. Из секции `logging`
различимы только `logger_active` и `lost_log_lines`.

**Соответствие «маршрут → конкретное право» держится только для пишущих
маршрутов.** `test_every_admin_route_is_guarded` проверяет, что обычному
пользователю отказано, а `test_read_only_admin_cannot_write` перечисляет
только пишущие. `/admin/health` — читающий, и подмена
`admin:view_system_health` на `admin:view_users` набор не роняет. Слова
`view_system_health` в `tests/` нет вовсе.

**У админских схем нет отрицательных случаев, кроме пароля и имени роли.**
Пережили: снятое `min_length=1` у `roles` и `permissions` (пустой список
снимает с пользователя все роли или обнуляет права роли), снятый шаблон
почты, шаблон без якорей (`"a b@c.d junk@"` проходит), `is_active` по
умолчанию `False` (админ заводит учётку, которая не может войти).

Ещё две отдельные точки: привязанные поля, перекрывающие одноимённые поля
события (аудит запишет пользователя из контекста запроса вместо
участника события), и `limit`/`offset`, переставленные местами в списке
пользователей (ответ `200` с пустым списком — ни один тест не смотрит
содержимое).
