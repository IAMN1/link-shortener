...
точки: `tests/unit` → `tests/build` даёт молча 477 вместо 1239). Один
...
оставляет сбор на 1239 и коде 0, и краснеет уже сам прогон.
...
неотслеживаемые файлы в него не попадут. Пока изменение не в ветке,
`--error-for-skips` в клоне окажется неизвестным флагом — плагина нет ни в
`pyproject.toml`, ни в `uv.lock` того среза.
