...
Тесты: 2106 (unit + integration + e2e), покрытие: 91.48% при пороге 88%
...
> **Живой прогон.** `uv run python tests/live/smoke_test.py` — 110 проверок
> по HTTP против настоящего приложения, не через pytest. Код возврата
> ненулевой, если хоть одна упала или если проверок оказалось не 110:
...
>
> **Браузерный прогон.** `uv run python tests/live/browser_test.py` — восемь
...
> восьми файлах роняет три проверки из восьми (`VALIDATION_ERROR`,
...
точки: `tests/unit` → `tests/build` даёт молча 687 вместо 2106). Два
...
оставляет сбор на 2106 и коде 0, и краснеет уже сам прогон.
...
| `tests/integration/web/controllers/test_auth_controller.py:226` | `test_unknown_account_is_not_answered_faster` — сравнивает **отношение** двух замеренных задержек, но берёт `min()` из трёх замеров и допускает десятикратную разницу, так что запас у него как раз широкий |
