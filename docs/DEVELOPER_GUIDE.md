...
Тесты: 2159 (unit + integration + e2e), покрытие: 91.66% при пороге 88%
...
точки: `tests/unit` → `tests/build` даёт молча 706 вместо 2159). Два
...
оставляет сбор на 2159 и коде 0, и краснеет уже сам прогон.
...
- **Адрес сравнивается посимвольно, включая регистр.** `Email` не приводит
  адрес к нижнему регистру, а `find_by_email` ищет точное совпадение, и
  уникальный индекс на `users.email` двум разным строкам не мешает.
  Замерено живыми запросами: после регистрации `case@example.com` запрос с
  `Case@Example.com` отвечает теми же `202` и заводит **вторую учётку** —
  в базе две строки на один почтовый ящик. Владелец получает два письма и
  может подтвердить обе. Обезличивания это не ломает (оба ответа
  одинаковы), но ломает предпосылку «один адрес — одна учётка», на которой
  стоят и подтверждение, и уборка неподтверждённых. Чинится нормализацией
  адреса, а это внешнее поведение: существующие учётки с заглавными
  буквами перестанут находиться по своей же строке, поэтому решение за
  владельцем
