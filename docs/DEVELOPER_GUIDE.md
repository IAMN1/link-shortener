...
### Значение, DTO или словарь

Три вида данных, и выбор между ними здесь не вкусовой.

**Value object** — когда у значения есть правила и равенство по значению:
`Email` приводит адрес к нижнему регистру и отказывает неверному,
`OriginalUrl` разбирает и проверяет URL, `ShortCode` знает, какие коды
служба не отдаёт. Такое значение нельзя передать строкой, не потеряв
проверку: строка не помнит, что её кто-то проверял. Семь штук в
`domain/value_objects/`; восьмой файл там, `verification_token.py`, класса
не объявляет — токен подтверждения живёт как пара функций, потому что
хранится только его дайджест.

**DTO** — когда набор полей пересекает границу слоя и поведения у него нет:
use case отвечает `ShortLinkResponse`, контроллер превращает его в
pydantic-схему ответа. Четырнадцать штук в `application/dtos/`. Сюда же
относятся снимки чтения: `ServiceLinkStats` и `UserLinkStats` объявлены
рядом со своим портом в `domain/repositories/link_repository.py`, как
`CachedRedirect` объявлен в порту кэша — контракт и его тип лежат вместе.

**Словарь** — только там, где словарь и есть цель: тело JSON, аргументы
логгера, запись в кэш, отображение вроде `Dict[UrlHash, Link]`. Это не
«данные с полями», а либо сериализация, либо коллекция.

Признак, по которому видно ошибку: слой отдаёт словарь, а другой слой
читает его по строковым ключам. Так было со статистикой —
`stats["total_clicks"] / stats["total_links"]`, где схему знал только
докстринг. Опечатка в ключе давала `KeyError` на странице статистики, а
через `.get(name, 0)` — молчаливый ноль. Теперь оба чтения идут через
`ServiceLinkStats` и `UserLinkStats`, и опечатка — ошибка mypy с
подсказкой верного имени.

Один такой путь остался, намеренно: `StatsCache.get_stats()` объявлен
`Optional[dict]`, и ветка попадания в кэш в `GetServiceStatsUseCase`
по-прежнему читает ключи строками. Там словарь — это записанный в Redis
JSON, и типом он станет только вместе со своей схемой сериализации; пока
этого нет, опечатка в той ветке проверкой типов не ловится.
...
**2476 тестов**, покрытие **94.86%** при пороге 88%. Менеджер пакетов — uv.
...
| 3 · e2e | Путь пользователя на SQLite, и он же на PostgreSQL с Redis | `uv run pytest tests/e2e/` |
...
│                                  # (in-memory SQLite и отдельно —
│                                  # тот же путь на PostgreSQL с Redis)
├── support/                       # Общее, что не является фикстурой
│                                  # одного каталога: подъём стека
├── live/                          # Smoke test всех эндпоинтов
└── load/                          # Профиль для locust, pytest его не собирает
```

**Категорию задаёт каталог, а не рука.** `pytest_collection_modifyitems` в
`tests/conftest.py` ставит марку по пути: `unit`, `integration`, `docker`
(для `integration/docker`), `e2e`. Написанная в файле марка могла бы
разойтись с местом, где файл лежит, а так `-m integration` выбирает ровно
то, что лежит в `integration/`. Прежний список объявлял ещё `slow`,
`database`, `cache` и `api` — их не ставил ни один тест из набора.

```bash
uv run pytest -m unit          # 1636, без внешних служб
uv run pytest -m integration   # 821, приложение и база
uv run pytest -m docker        # 38, нужен поднятый стек
uv run pytest -m e2e           # 19, пути пользователя
```

Оговорка про `unit`: 103 теста под `tests/unit/web/` собирают настоящее
приложение через `create_app` — с настоящими блюпринтами, middleware и
картой URL, — но все службы контейнера в них заглушки (`tests/unit/web/
conftest.py`). Внешнего они не поднимают, поэтому лежат здесь; «только
моки» было бы неточно.

**Три уровня — это три разных вопроса.** `unit` спрашивает «делает ли эта
единица то, что обещает», `integration` — «сходятся ли слои», а `docker` и
`e2e` на настоящем стеке — «работает ли это на том, что развёрнуто».
Последнее не выводится из первых двух:
`tests/e2e/test_journey_on_real_stack.py` при первом же прогоне нашёл, что
вспомогательная функция набора писала `email_verified = 1` — SQLite берёт
целое за истину, PostgreSQL отказывает.
...
flowchart TD
    ...
    subgraph S["Обе половины матрицы"]
        ...
        L2 --> L3[Образы тестового стека]
        L3 --> L4[Счёт собранных тестов<br/>минимум 2470]
        L4 --> L5[Прогон набора<br/>--error-for-skips]
        L5 --> L6[Живой прогон<br/>114 проверок]
    end

    subgraph T["Линтеры, один раз"]
        F[flake8] --> P[pylint] --> B[bandit] --> M[mypy]
    end
...
| `pylint src` | `fail-under = 9.0` из `pyproject.toml` | 9.20 |
| `bandit -r src -q` | Только `src`: `assert` для bandit находка, а для теста смысл существования | 0 находок, семь помечены `# nosec` с разбором каждой |
| `mypy src` | Настройки — `[tool.mypy]` в `pyproject.toml`; порога нет, ворота стоят на нуле. `check_untyped_defs` включён, иначе тела функций без аннотаций не проверяются вовсе | 0 ошибок в 279 файлах, двенадцать мест помечены `# type: ignore` с причиной |

Локальное число сверяется тем же прогоном, что делает CI на свежем клоне, —
с нуля, без кэша:
...
Путь до нуля начинался со 143 ошибок в 63 файлах — замер на `dev`, под
умолчаниями mypy, до того как появилась секция `[tool.mypy]`. Большинство
закрыто тем, что объявления стали описывать код (`UnitOfWorkFactory` вместо
`Callable[[], UnitOfWork]`, `ServiceCache` вместо `LinkCache`, `Optional`
там, где значение бывает `None`). Остаток погашен двенадцатью метками —
снимите их все, и mypy сообщит ровно 14 ошибок в 8 файлах:

| Сколько | Где | Почему не правка кода |
|---------|-----|------------------------|
| 7 | `production.py`, `staging.py`, `testing.py` | Профиль превращает записываемую настройку базового класса в read-only property, и это его назначение |
| 2 | `app_factory.py`, `cli/adapters/flask.py` | `container` кладёт на приложение `create_app`; у Flask такого атрибута нет. Двадцать девять мест — 27 команд и 2 хелпера — читают его через один `_container()`, поэтому меток две, а не тридцать |
| 2 одной меткой | `celery/celery_config.py` | Протокол дескриптора объявляет `owner` как `type \| None`, и поля нет ни у одной половины |
| 2 одной меткой | `rate_limit/redis_rate_limiter.py` | Один класс redis-py обслуживает синхронный и асинхронный клиент, поэтому `eval` объявлен возвращающим ещё и awaitable |
| 1 | `use_cases/auth/register.py` | Токен пишется в той же ветке, что и пользователь, а проверяется только пользователь |

Ещё три ошибки из тех 143 закрыты правкой кода там, где напрашивалась
метка: `_RaisesForOwnRecords` объявлен над `logging.Handler` — он и так
зовёт `super().handleError`, — а разбор IPv4 принимает `Optional[str]`,
который ему и передают, и держит отфильтрованный список под своим именем
вместо того, чтобы объяснять в комментарии, почему `None` в нём уже нет.

`warn_unused_ignores` не даёт метке пережить свою причину: подавление,
ставшее ненужным, роняет прогон. Чего он не ловит — снятия с проверки
целиком: строка `# mypy: ignore-errors` в начале файла или
`ignore_errors = true` в `[[tool.mypy.overrides]]` уводят из-под ворот файл
или весь пакет, и прогон остаётся зелёным. Это видно только в диффе, так
что смотреть на такие строки — работа ревью, а не настройки.

Отдельно про `check_untyped_defs`. По умолчанию mypy не заходит в тела
функций без аннотаций и сообщает об этом заметкой, а не ошибкой, — то есть
ворота молчали бы про `di/container.py` и CLI-адаптер целиком. Замерено на
том состоянии ветки, где остальные ошибки были уже закрыты: без флага —
ноль, с флагом — 39 ошибок в 5 файлах. Восемь из них про кэш: контейнер
раздавал его как `LinkCache` туда, где объявлены роли — `ServiceCache`,
`StatsCache`, `RedirectCache`.

Настройки держат друг друга: снимите сегодня `check_untyped_defs`, и прогон
упадёт с `Unused "type: ignore"` в `redis_rate_limiter.py` — метка нужна
ровно потому, что тело той функции проверяется.

Два инструмента остались вне ворот, и это решение, а не забывчивость:

| Инструмент | Замер | Почему не в CI |
|------------|-------|----------------|
| `black` | переписал бы 352 файла из 484 | Ворота означали бы переверстать проект под инструмент |
| `isort` | 212 файлов расходятся даже с `--profile=black --lines-after-imports=2` | То же самое |

Оба остались в группе `dev` и запускаются руками.
...
`flake8` проходит 484 файла за 0,6 секунды. Новая зависимость ради
...
- `[tool.flake8]` в `pyproject.toml` — flake8 этот файл не читает вовсе, и
  записанный предел строки 88 не действовал: на 14 августа 2026 это 1269
  замечаний `E501` при умолчании в 79 символов против 269 при заявленных
  88. Конфигурация переехала в `.flake8`;
