...
пойдёт туда же. Замерено: `production` с заданными секретами и `DOMAIN`,
но без `DATABASE_TYPE`, поднимается и мигрирует `db_shortener` в корне.
Команда при этом печатает `Database: ...`, так что цель видна — но
защиты здесь нет, и правильное место для неё не миграция, а отказ
приложения подниматься на такой базе. Записано как открытый вопрос.
