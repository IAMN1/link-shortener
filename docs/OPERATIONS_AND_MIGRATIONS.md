...
> `sqlite:////<корень>/db_shortener`. Теперь такой старт отвергается, и
> сообщение называет базу, на которую профиль смотрел.
>
...
> одним потоком, второму не видна — тоже замерено. Миграция три
> последние формы отвергает и сама (см. «Миграция без приложения»).
>
...
> они нужны.
>
> `development` на этом файле работает как работал: там он и задуман.
> Неназванный профиль — тоже `development`, и в отличие от миграции
