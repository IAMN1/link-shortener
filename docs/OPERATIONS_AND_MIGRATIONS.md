...
Так было не всегда, и это стоило дорого. Замерено на профиле `production`
с уже заданным `DATABASE_URL`: команда отказывала четыре раза подряд —
`SECRET_KEY`, затем `SHORT_CODE_PEPPER`, затем `REDIS_URL`, затем
`DOMAIN`, — и только пятый запуск создавал таблицы. `MAIL_ENABLED=true` с
неполными настройками почты и `MAX_URL_LENGTH` больше 2048 останавливали
её так же надёжно. Тот же отказ ронял docker-стек целиком: сервис
`migrations` не выполнялся, а `app` ждёт его успешного завершения.
...
упущением, которое ищет: замерено — миграция молча создавала
`db_shortener` в корне проекта и выходила с нулём.
...
и `DATABASE_STATEMENT_TIMEOUT` применяются и к ней. Раньше — нет: свой
engine она строила из секции `[alembic]`, где нет ничего кроме URL.
Замерено против недоступного сервера при `DATABASE_CONNECT_TIMEOUT=3`:
приложение сдавалось за 3.6 с, миграция не сдалась за 60 — а в стеке `app`
ждёт её завершения, так что висел весь стек.
...
| `flask maintenance normalize-emails` | Привести к нижнему регистру адреса, записанные до того, как `Email` стал нормализовать. Без `--apply` только показывает, что изменится. Пару, схлопывающуюся в один адрес, не трогает и выводит отдельно: слияние двух учёток — решение владельца данных. Разовая команда для существующей базы, в расписание не нужна. Единственное место, где регистр адреса меняется: приложение такие строки оставляет как есть и пишет о них предупреждение — см. врезку ниже |
...
> упиралась в уникальный индекс. Замерено на подтверждении адреса —
...
> деактивация администратором падала тем же образом.
>
> Теперь строка остаётся ровно такой, как записана, а в журнал уходит
...
> данных, а не команды обслуживания. Замерено: после `--apply` одиночная
> строка исчезает из отчёта, парная остаётся и предупреждает дальше.
...
> лимита.
>
> Раньше `RATE_LIMITS` при этом объявлял для него `(10, 5)`, и запись
> молча не работала: проба за пробой, ни одного `429`. Настройка читалась
> как включённая защита, которой не было. Теперь каждый ключ
> `RATE_LIMITS` сверяется с таблицей маршрутов при старте, и приложение
> **не поднимется**, если ключ недостижим — освобождён, отдаётся из
> `/static/` или просто ни на что не указывает. Проверяются и значения:
> всё, что не пара положительных целых, раньше доживало до первого
> запроса — и ломалось тремя разными способами. `10`, `(5,)` и `("5", 60)`
> отвечали `500` на каждый запрос к своему эндпоинту. Нулевой лимит
> `(0, 60)` отказывал **всем и всегда**. Отрицательный период `(5, -60)`
> уводил начало окна в будущее, поэтому каждая отметка оказывалась вне
> окна и не троттлилось **вообще ничего** — ровно тот дефект, ради
> которого проверка написана, только записанный иначе. Сообщение называет
> каждый негодный ключ и причину.
>
...
>
> **Опечатка в имени эндпоинта — самый вероятный случай, и раньше он был
> и самым тихим.** `auth.login`, написанный как `auth.log_in`, не даёт
> никакой ошибки: эндпоинт продолжает отвечать, но по умолчанию — и
> защита от перебора пароля падает с пяти попыток в минуту до ста.
>
...
> есть.** Раньше путь читался от рабочего каталога процесса — SQLAlchemy
> отдаёт относительный путь Python и о каталоге не говорит ничего, —
> поэтому запуск из `src/` открывал не ту базу, а **создавал** новую
> пустую: отсутствующий файл SQLite не ошибка, а повод его завести.
...
> reference, раздел `ports`). Раньше база публиковалась именно так,
> тогда как оба Redis были привязаны к петле; замерено на тестовом стеке —
> порт отвечал с адреса машины в локальной сети, после правки не отвечает.
...
> `db_shortener`, поэтому развёртывание, забывшее задать базу,
> поднималось на пустом новом файле в корне проекта и отвечало так, будто
> данных никогда не было; замерено на профиле `production`, где всё
> остальное задано — `SECRET_KEY`, `SHORT_CODE_PEPPER`, `DOMAIN`, а
> `REDIS_ENABLED` выключен, — конфигурация проходила проверку и открывала
> `sqlite:////<корень>/db_shortener`. В образе корня проекта нет, и файл
> ложится в рабочий каталог (`WORKDIR /app`) — почему так, объясняет
> врезка про привязку пути ниже. Теперь такой старт отвергается, и
...
> поэтому MySQL или опечатка в схеме (`postgres://` вместо
> `postgresql://` — SQLAlchemy 2.0 такую не грузит) тоже останавливают
> старт.
>
...
> `celery` и `flask` разный — замерено: миграция записала 147456 байт в
> один файл, приложение открыло пустой рядом. База в памяти
> (`sqlite://`, `sqlite:///:memory:`) выдаёт каждому потоку свою копию:
> под выбранным для неё пулом `SingletonThreadPool` таблица, созданная
> одним потоком, второму не видна — тоже замерено.
>
...
> она мигрирует — замерено, `sqlite:///relative.db` под `production` даёт
...
> читать. Замерено: с `FLASK_ENV=production` в одном лишь
> `.env.production` приложение остаётся на `development` и спокойно
> стартует на SQLite, то есть развёртывание, настроенное так, проверки
> выше не увидит вовсе. Профиль называют переменной окружения или в
> `.env`.
