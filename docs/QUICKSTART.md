...
| `this profile runs on PostgreSQL` | Профили `production` и `staging` работают только на PostgreSQL, а конфигурация назвала другой бэкенд — по умолчанию это пустой новый файл SQLite в корне проекта. Задайте `DATABASE_TYPE=postgresql` вместе с частями `DATABASE_*` либо целый URL в `DATABASE_URL`. Отказ одинаков для файла, названного в `DATABASE_NAME`, для файла в `DATABASE_URL` и для базы в памяти. В `development` SQLite остаётся доступным |
