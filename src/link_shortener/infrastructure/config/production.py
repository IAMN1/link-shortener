...
class ProductionConfig(BaseConfig):
    ...
    def SHORT_CODE_SECRET_PEPPER(self) -> str:
        ...
        if not pepper:
            raise ValueError('SHORT_CODE_PEPPER должен быть установлен в переменных окружения')
    ...
    def BASE_URL(self) -> str:
        ...
        if domain:
            scheme = 'https' if os.environ.get('USE_HTTPS', True) is True else 'http'
    ...
    def DATABASE_URL(self) -> str:
        ...
        if not url:
            raise ValueError('DATABASE_URL должен быть установлен в переменных среды коружения')
    ...
    REDIS_ENABLED: bool = os.environ.get('REDIS_ENABLED', True)
    ...
    def __init__(self):
        """Дополнительная инициализация для продакшена"""
        required_vars = [
            'SECRET_KEY', 'SHORT_CODE_SECRET_PEPPER',
