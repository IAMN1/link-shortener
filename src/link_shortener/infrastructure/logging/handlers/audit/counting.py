...
A wrapper rather than a call at each site that writes: the vocabulary is
written from the use cases, the error handler and the CLI, and a counter
invoked beside each `audit.log_*` is a counter that the next one forgets.
Here, every event is counted by construction, and the next one is counted
without a line of proof being needed anywhere -- which is how
