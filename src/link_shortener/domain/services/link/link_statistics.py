...
class LinkStatistics(BaseService):
    ...
    def get_service_stats(self) -> ServiceStatsResult:
        ...
        if self._cache_manager and self._stats_strategy:
            success = self._cache_manager.cache_service_stats(
                result, self._stats_strategy, self._cache_ttl
            )
            if success:
                self._log_debug("Статистика успешно закэширована")

        self._log_info(
            "Статистика получена", total_urls=total_urls, total_clicks=total_clicks
        )
    ...
    def service_stats_with_details(
        self, base_url: str, include_details: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        for url_info in stats.popular_urls:
            ...
            url_with_short["short_url"] = (
                f"{base_url.rstrip('/')}/{url_info['short_code']}"
            )
