...
class LinkRedirector(BaseService):
    ...
    def extract_short_code(self, short_url: str) -> str:
        ...
        self._log_debug(
            "Код успешно извлечен", short_url=short_url, short_code=short_code
        )
        return short_code
    ...
    def get_original_url(self, short_url: str) -> RedirectResult:
        ...
        if self._cache_manager and self._redirect_strategy:
            cached_url = self._cache_manager.get_original_url(
                short_code, self._redirect_strategy
            )
            ...
            if cached_url:
                ...
                return RedirectResult(
                    original_url=cached_url, from_cache=True, clicks=None
                )
        ...
        if not success:
            self._log_error(
                "Не удалось обновить счетчик переходов", short_code=short_code
            )
        ...
        return RedirectResult(
            original_url=link.original_url, from_cache=False, clicks=link.clicks
        )
                ...
                if self._cache_manager:
                    ...
                    if strategies:
                        self._cache_manager.cache_link(
                            link, strategies, self._cache_ttl
                        )
                        self._log_debug("Кэш успешно обновлен", short_code=short_code)

        except Exception as e:
            self._log_error(
                "Ошибка при обновлении кэша", short_code=short_code, error=str(e)
            )
