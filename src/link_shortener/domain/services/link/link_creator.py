...
class ShortLinkCreator(BaseService):
    ...
    def create_short_url(self, original_url: str) -> ShortLinkCreationResult:
        ...
        if self._cache_manager and self._hash_strategy:
            cached_link = self._cache_manager.get_link_by_hash(
                url_hash, self._hash_strategy
            )

            if cached_link:
                self._log_info(
                    "Ссылка найдена в кэше", short_code=cached_link.short_code
                )
                return ShortLinkCreationResult(
                    link=cached_link, is_new=False, from_cache=True
                )
        ...
        if existing_link:
            self._log_info(
                "Ссылка найдена в базе данных", short_code=existing_link.short_code
            )
            ...
            if self._cache_manager and self._hash_strategy and self._redirect_strategy:
                ...
                self._cache_manager.cache_link(
                    existing_link, strategies, self.cache_ttl
                )

            return ShortLinkCreationResult(
                link=existing_link, is_new=False, from_cache=False
            )
        ...
        if self._cache_manager and self._hash_strategy and self._redirect_strategy:
            ...
            success = self._cache_manager.cache_link(
                saved_link, strategies, self.cache_ttl
            )

            if success:
                self._log_debug("Ссылка закэширована")

        return ShortLinkCreationResult(link=saved_link, is_new=True, from_cache=False)
